  ...
  const MAP = {
    ...
    members: {
      ...
      toObj: r => ({ nick: r[0] || "", gender: (r[1] || "").indexOf("여") >= 0 ? "여자" : "남자",
                     ...
                     bday: r[16] || "" }), // 생일 "MM-DD"(연도 없음). 빈값=비공개/미입력
      toRow: m => [m.nick, m.gender, m.age, m.region, m.height, m.job, m.off, m.hobby, m.mbti, m.charm, m.ideal, m.smoke, m.say, m.type || "미분류", m.pw || "", m.at || "", m.bday || ""],
      keep: x => !!x.nick
    },
    patchnotes: {
      head: ["날짜", "분류", "버전", "내용"],
    ...
    notices: {
      head: ["날짜", "제목", "내용", "고정"],
      toObj: r => ({ date: normDate(r[0]), title: r[1] || "", body: r[2] || "",
